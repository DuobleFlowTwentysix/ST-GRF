...
class STGRF_BIGRU(nn.Module):
    def __init__(self, hist_len, pred_len, in_dim, city_num, batch_size, device, edge_index, edge_attr, wind_mean, wind_std):
        ...
        self.gru_cell = nn.GRU(input_size=self.in_dim + self.gnn_out, hidden_size=self.hid_dim,batch_first=True,num_layers=self.num_layers,bidirectional=True)
